...
reverse :: proc(node: ^parser.Node) {
	switch &v in node {
	case ^parser.Node_Rune, ^parser.Node_Rune_Class, ^parser.Node_Wildcard:
	case ^parser.Node_Concatenation:
		for &n in v.nodes {
			reverse(&n)
		}
		slice.reverse(v.nodes[:])
	...
	case ^parser.Node_Group:
		reverse(&v.inner)
	case ^parser.Node_Anchor:
		v.start ~= true
	case ^parser.Node_Word_Boundary:
	case ^parser.Node_Match_All_And_Escape:
	}
}
